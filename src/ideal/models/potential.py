...
from .m3gnet.utils.build import build_dataloader
from .m3gnet.utils.download import download_checkpoint
...
class Potential(nn.Module):
    ...
    def finetune_mode(
        self,
        finetune_layers: int = -1,
        finetune_head: nn.Module | None = None,
        reset_head_for_finetune: bool = False,
    ...
    def train_model(
        ...
        val_dataloader,
        loss: torch.nn.modules.Module = torch.nn.MSELoss(),
        ...
        for epoch in range(self.last_epoch + 1, epochs):
            ...
            if need_to_load_data:
                ...
                for idx, data_path in enumerate(dataloader):
                    ...
                    atoms_train_sampler = DistributedSampler(
                        train_data,
            ...
            self.validation_metrics = {
                "loss": metric[0],  # type: ignore
                "MAE_energy": metric[1],  # type: ignore
                "MAE_force": metric[2],  # type: ignore
                "MAE_stress": metric[3],  # type: ignore
            }
            if is_distributed:
        ...
        with self.ema.average_parameters():
            try:
                ...
                if (
                    ...
                    self._save(os.path.join(save_path, "best_model.pth"))
            ...
            if save_checkpoint is True and epoch % ckpt_interval == 0:
                self._save(os.path.join(save_path, f"ckpt_{epoch}.pth"))
            if save_checkpoint is True:
                self._save(os.path.join(save_path, "last_model.pth"))
            return False
    ...
    def train_one_epoch(
        ...
        **kwargs,
        ...
        for batch_idx, graph_batch in enumerate(dataloader):
            ...
            if include_energy:
                e_mae_list.extend(e_mae)
            if include_forces:
                f_mae_list.extend(f_mae)
            if include_stresses:
                s_mae_list.extend(s_mae)

        loss_avg_ = loss_avg.compute().item()
        if include_energy:
            e_mae = sum(e_mae_list) / len(e_mae_list)
        else:
            e_mae = 0
        if include_forces:
            f_mae = sum(f_mae_list) / len(f_mae_list)
        else:
            f_mae = 0
        if include_stresses:
            s_mae = sum(s_mae_list) / len(s_mae_list)
        else:
            s_mae = 0
        ...
        if reduction == "mean":
            ...
            return loss_avg_, e_mae_list, f_mae_list, s_mae_list
        else:
            raise NotImplementedError
    ...
    def loss_calc(
        ...
        e_mae = [0] * graph_batch.num_graphs
        f_mae = [0] * graph_batch.num_graphs
        s_mae = [0] * graph_batch.num_graphs
        ...
        if self.model_name == "graphormer" or self.model_name == "geomformer":
            ...
            if include_energy:
                ...
                e_mae = torch.abs(e_pred - e_gt).detach().cpu().numpy()
                e_mae = e_mae.reshape(-1).tolist()
            if include_forces:
                f_gt = graph_batch.forces
                f_pred = result["forces"]
                loss_ = loss_ + loss(f_pred, f_gt) * loss_f
                f_mae = torch.mean(torch.abs(f_pred - f_gt), dim=1)
                f_mae = scatter(
                    f_mae,
                    graph_batch.batch,
                    dim=0,
                    dim_size=graph_batch.num_graphs,
                )
                f_mae = f_mae / graph_batch.num_atoms
                f_mae = f_mae.detach().cpu().numpy().reshape(-1).tolist()
            if include_stresses:
                s_gt = graph_batch.stress
                s_pred = result["stresses"]
                loss_ = loss_ + loss(s_pred, s_gt) * loss_s
                s_mae = (
                    torch.mean(torch.abs(s_pred - s_gt), dim=(1, 2))
                    .detach()
                    .cpu()
                    .numpy()
                )
                s_mae = s_mae.reshape(-1).tolist()
