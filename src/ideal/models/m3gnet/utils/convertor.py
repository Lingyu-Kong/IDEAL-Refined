...
def compute_threebody_indices(
    ...
    """
    n_atoms = np.array(n_atoms).reshape(1)  # type: ignore
...
def get_fixed_radius_bonding(
    ...
    lattice_matrix = np.ascontiguousarray(np.array(structure.cell)[:], dtype=float)  # noqa: E501
        ...
        **kwargs,
        ...
        if isinstance(atoms, Atoms):
            ...
            if np.all(pbc_ < 0.01) or not pbc:
                min_x = np.min(atoms.positions[:, 0]).astype(float)
                min_y = np.min(atoms.positions[:, 1]).astype(float)
                min_z = np.min(atoms.positions[:, 2]).astype(float)
                max_x = np.max(atoms.positions[:, 0]).astype(float)
                max_y = np.max(atoms.positions[:, 1]).astype(float)
                max_z = np.max(atoms.positions[:, 2]).astype(float)
