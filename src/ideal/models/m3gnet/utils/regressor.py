...
def solver(X, y, regressor: str | None = "NormalizedGaussianProcess", **kwargs):  # noqa: E501
...
def normalized_gp(X, y, **kwargs):
    ...
    feature_rms = np.nan_to_num(feature_rms, nan=1)
    y_mean = torch.sum(y) / torch.sum(X)
    mean, std = base_gp(
        X,
        y - (torch.sum(X, dim=1) * y_mean).reshape(y.shape),
...
def base_gp(
    ...
    while not_fit:
        ...
        try:
            ...
            alpha_array = np.ones(y.shape[0]) * alpha
            gpr = GaussianProcessRegressor(
                kernel=_kernel, random_state=0, alpha=alpha_array
            )  # noqa: E501
            gpr = gpr.fit(X, y)

            vec = torch.diag(torch.ones(X.shape[1]))
            mean, std = gpr.predict(vec, return_std=True, return_cov=False)  # type: ignore

            mean = torch.as_tensor(mean, dtype=torch.get_default_dtype()).reshape(  # noqa: E501
                [-1]
            )  # noqa: E501
            # ignore all the off-diagonal terms
            std = torch.as_tensor(std, dtype=torch.get_default_dtype()).reshape(  # noqa: E501
                [-1]
            )  # noqa: E501
            ...
            if iteration >= max_iteration or not_fit is False:
                raise ValueError(
                    "Please set the per species shift and scale "
                    "to zeros and ones. \nThe dataset energy is "
                    "to diverge to be solved with GP"
                )
...
class NormalizedDotProduct(Kernel):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if Y is None:
            ...
            if eval_gradient:
                raise ValueError(
                    "Gradient can only be evaluated when Y is None."  # noqa: E501
                )
    ...
    def diag(self, X):
        ...
        """
        return np.einsum("ij,ij,jj->i", X, X, self.A)  # type: ignore
