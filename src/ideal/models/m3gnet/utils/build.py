...
from .convertor import GraphConvertor
...
def build_dataloader(
    ...
    **kwargs,
    ...
    length = len(atoms)
    if energies is None:
        energies = [None] * length  # type: ignore
    if forces is None:
        forces = [None] * length  # type: ignore
    if stresses is None:
        stresses = [None] * length  # type: ignore

    if model_type == "m3gnet":
        if multiprocessing == 0 and multithreading == 0:
            # start = time.time()
            for graph, energy, force, stress in zip(atoms, energies, forces, stresses):  # type: ignore
            ...
            preprocessed_data = pool.starmap(
                convertor.convert,
                zip(atoms, energies, forces, stresses),  # type: ignore
            )
            pool.close()
...
def cell_expand(pos, atoms, cell, cutoff=10.0):
    ...
    max_expand_len = int(torch.max(expand_len).item())
