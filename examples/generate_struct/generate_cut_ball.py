...
def generate_bcc_Fe_nanoparticle(
    radius: float, nanoparticle_element: str, box_extension: float
) -> Atoms:
    bcc_bulk = bulk(nanoparticle_element, crystalstructure="bcc", a=2.866)
    Nx = int(np.ceil(2.0 * radius / 2.866) + 1) * 2
    Ny = Nx
    Nz = Nx
    supercell = make_supercell(bcc_bulk, [[Nx, 0, 0], [0, Ny, 0], [0, 0, Nz]])
    bulk_pos = np.array(supercell.get_positions())
    bulk_cell = np.array(supercell.get_cell())
    bulk_symbols = supercell.get_chemical_symbols()
    bulk_center = 0.5 * (bulk_cell[0] + bulk_cell[1] + bulk_cell[2])
    positions = supercell.get_positions()
    distances = np.linalg.norm(positions - bulk_center, axis=1)
    inside_mask = distances <= radius
    positions = bulk_pos[inside_mask]
    symbols = [bulk_symbols[i] for i in range(len(bulk_symbols)) if inside_mask[i]]
    box_size = 2 * radius + box_extension
    box_cell = np.eye(3) * box_size
    box_center = 0.5 * (box_cell[0] + box_cell[1] + box_cell[2])
    offset = box_center - np.mean(positions, axis=0)
    positions += offset
    nanoparticle = Atoms(symbols, positions=positions, cell=box_cell, pbc=True)
    return nanoparticle
...
def generate_hcp_Ru_nanoparticle(
    ...
    a = 2.706
    c = 4.28
    hcp_bulk = bulk(nanoparticle_element, crystalstructure="hcp", a=a, c=c)
    nx = int(np.ceil(2 * radius / a) + 1) * 2
    ny = int(np.ceil(2 * radius / (np.sqrt(3) * a / 2)) + 1) * 2
    nz = int(np.ceil(2 * radius / c) + 1) * 2
    supercell = make_supercell(hcp_bulk, [[nx, 0, 0], [0, ny, 0], [0, 0, nz]])
    bulk_cell = np.array(supercell.get_cell())
    bulk_pos = np.array(supercell.get_positions())
    bulk_symbols = supercell.get_chemical_symbols()
    center = 0.5 * (bulk_cell[0] + bulk_cell[1] + bulk_cell[2])
    positions = supercell.get_positions()
...
if __name__ == "__main__":
    ...
    if nanoparticle_element == "Fe":
        cluster = generate_bcc_Fe_nanoparticle(
            radius, nanoparticle_element, box_extension
        )
    else:
    ...
    num_promoters = int(len(cluster) * promoters_ratio)
    ...
    final_name = f"../../contents/habor-bosch/{nanoparticle_element}{final_np_size}-{promoter_element}{final_num_promoters}-{add_promoter_method}-{final_H2_pressure}atmH2-{final_N2_pressure}atmN2.xyz"
